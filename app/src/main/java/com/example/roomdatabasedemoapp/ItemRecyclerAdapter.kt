...
class ItemRecyclerAdapter(private val list: ArrayList<EmployeeEntity>):
    RecyclerView.Adapter<ItemRecyclerAdapter.ViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val v = LayoutInflater.from(parent.context).inflate(R.layout.item_employee_entry, parent, false)
        return ViewHolder(v)
    }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (position % 2 == 0) {
            holder.parentLayout.setBackgroundColor(ContextCompat.getColor(holder.itemView.context, R.color.gray))
        } else {
            holder.parentLayout.setBackgroundColor(ContextCompat.getColor(holder.itemView.context, R.color.white))
        }
    }
